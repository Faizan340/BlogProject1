...
class BlogDetail(DetailView):
    ...
    def get_context_data(self,  *args, **kwargs):
        ...
        comments = BlogCommentModel.objects.order_by(
            '-created_at').filter(blogpost=blog_id)[:5]
        ...
        rating_avg = BlogRatingModel.objects.filter(
            post=blog_id).aggregate(Avg('rating'))
        rate = {}
        rate = rating_avg
...
class BlogHome(ListView):
    ...
    def get_context_data(self,  *args, **kwargs):
        context = super().get_context_data(**kwargs)
        blog_list = PostBlog.objects.order_by(
            '-created_at').filter(status=1)[:4]
        count = PostBlog.objects.filter(status=1).count()
        lst = []
        publish = PostBlog.objects.filter(status=1)
        posts = PostBlog.objects.all()
        for p in posts:
            rating = BlogRatingModel.objects.filter(
                post=p.id).aggregate(Avg('rating'))
